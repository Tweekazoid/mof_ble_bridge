...
THIS_REPO = os.path.abspath(os.path.join(os.path.dirname(__file__), "../mof_ble_bridge")).replace(
    "\\",
    "/",
)
...
RESULTING_ADDON_NAME = "mof_ble_bridge_dev"

# \scripts\addons

all_versions = [p.replace("\\", "/") for p in glob.glob(BLENDER_VERSIONS_PATH + "/*/scripts/addons")]
...
def _try_junction(src: str, dst: str) -> bool:
    ...
    try:
        ...
        if proc.returncode == 0:
            return True
        LOG.error("  Junction failed: %s", proc.stderr.strip() or proc.stdout.strip())
    except Exception:
        LOG.exception("  Junction failed:")
        return False
    else:
        return False
...
for version_path in all_versions:
    ...
    if not match:
        LOG.error("Could not parse Blender version from path: %s", version_path)
        continue
    version = match.group(1)
    target_addon_path = os.path.join(version_path, RESULTING_ADDON_NAME).replace("\\", "/")
    LOG.info("Setting up link for Blender %s -> %s", version, target_addon_path)
    try:
        _remove_existing(target_addon_path)

        if _try_junction(THIS_REPO, target_addon_path):
            LOG.info("Linked (junction) Proxor addon to Blender %s addons folder.", version)
            continue

        LOG.error("Failed to set up addon for Blender %s. See errors above.", version)
    except Exception:
        LOG.exception("Failed to link for Blender %s", version)
